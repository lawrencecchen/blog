...
- https://x.com/superconductdev
- https://conductor.build/

I still decided to work on cmux to scratch my own itch. I also had some specific needs that other wrappers didn't entirely satisfy, so it made sense to build my own. Those include:

- Open source
- Each instance runs its own VS Code instance, so we can benefit from Claude Code's VS Code extension /ide integration. Applies to OpenCode as well.
...
Not all of these are implemented yet, but I want to make it happen.
